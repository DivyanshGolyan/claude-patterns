...
```bash
cluster-claude-messages user_messages.json --threshold 0.7
```

**If running directly:**

```bash
uv run python -m claude_patterns.clustering user_messages.json --threshold 0.7
```

Outputs: `clusters_YYYYMMDD_HHMMSS/` folder with:
...
Outputs: Slash command files in `.claude/commands/`
